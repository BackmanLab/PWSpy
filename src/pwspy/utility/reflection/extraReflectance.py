...
def getAllCubeCombos(matCombos: t_.Iterable[MCombo], cubes: t_.Iterable[t_.Tuple[Material, ImCube]]) -> t_.Dict[MCombo, t_.List[CubeCombo]]:
    ...
    Args:
        ...
        cubes: An iterable containing pairs of a Material enumerator and an ImCube that was measured at that material.
    ...
    for matCombo in matCombos:
        matCubes = {}  # A dictionary with materials as keys and lists of associated imcubes as the values.
        for material in matCombo:
            matCubes[material] = [cube for mat, cube in cubes if mat == material]
...
def plotExtraReflection(df: pd.DataFrame, theoryR: t_.Dict[Material, pd.Series], matCombos: t_.List[MCombo],
    ...
    for sett in settings:
        ...
        cubeCombos = getAllCubeCombos(matCombos, matCubeIterator)
