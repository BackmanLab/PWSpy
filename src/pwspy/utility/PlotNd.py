...
class PlotNd(object):
    ...
    def __init__(self, X: np.ndarray, names=('y', 'x', 'lambda'), initialCoords=None, title=''):
        ...
        self.line4 = [ax['extra'][i].plot([self.min, self.max], [100, 100], 'r', linewidth=lw)[0] for i in
                      range(self.extraDims)]
        self.yplot = ax['y'].plot(self.X[(slice(None),) + self.coords[1:]], np.arange(self.X.shape[0]))[0]
        self.xplot = ax['x'].plot(np.arange(self.X.shape[1]), self.X[(self.coords[0], slice(None)) + self.coords[2:]])[
            0]
        self.zplots = [ax['extra'][i].plot(self.X[self.coords[:2 + i] + (slice(None),) + self.coords[3 + i:]],
                                           np.arange(self.X.shape[2 + i]))[0] for i in range(self.extraDims)]
    ...
    def update(self):
        ...
        [self.zplots[i].set_data(self.X[self.coords[:2 + i] + (slice(None),) + self.coords[3 + i:]],
                                 np.arange(self.X.shape[2 + i])) for i in range(self.extraDims)]
        ...
        [self.line4[i].set_data(self.line4[i].get_data()[0], [self.coords[2 + i], self.coords[2 + i]]) for i in
         range(self.extraDims)]
