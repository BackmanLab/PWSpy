...
def proc(q, mmask, sref, theory):
    ...
    zim = (im.data - im.data.mean(axis=2, keepdims=True)) / (
        im.data.std(axis=2, keepdims=True))  # zero normalize for correlation
...
if __name__ == "__main__":
    # %%User Input
    fileNameTranslator = {'aclosefopen': "Aperture Closed", 'alclosefopen': "Aperture Barely Open",
                          'aopenfopen': "Aperture Open", 'alopenfopen': "Aperture Barely Closed"}
    files = glob('G:/Data/thinfilmcomparisonround2/a*')
    files = [i for i in files if ('fmid' not in i) and ("fclose" not in i)]
    # %% Loading
    theory = pd.read_csv(os.path.join(os.path.split(__file__)[0], 'thinFilmData', 'Reflectance-calcs.txt'),
                         delimiter='\t',
    ...
    # %% Processing
    result = loadAndProcess(files, processorFunc=proc, specifierNames=['filename'], parallel=True,
                            procArgs=(mmask, sref, theory))
