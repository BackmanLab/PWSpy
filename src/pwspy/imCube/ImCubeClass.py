...
class ImCube(ICBase, ICMetaData):
    ...
    def __init__(self,data, metadata:dict, dtype = np.float32, filePath = None):
        ...
        if all([i in self.metadata for  i in ['darkCounts', 'linearityPoly']]):
            self.cameraCorrection = CameraCorrection(darkCounts = self.metadata['darkCounts'], linearityPolynomial = self.metadata['linearityPoly'])
        else:
    ...
    def correctCameraEffects(self, correction:'CameraCorrection'=self.cameraCorrection, binning:int=None):
