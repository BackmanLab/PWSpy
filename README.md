...
## Statement of need
Any analysis of raw data generated by PWS and related imaging modes requires loading image data and metadata from a large variety of file formats, performing complex pre-processing for the purposes of normalization and calibration all before any of the real analysis begins. Any minor deviations to how this pre-processing is performed can result in major differences in analysis results. PWSpy provides an object-oriented interface for performing all commonly performed tasks in analysis of PWS data. This allows users to write succinct and readable scripts/software that function identically regardless of the file format of the raw data and which are guaranteed to perform all operations properly.

## Documentation
Documentation is hosted online at [ReadTheDocs](https://pwspy.readthedocs.io/en/dev/)
