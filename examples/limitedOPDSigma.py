...
#   access cell data
for cell = cells     
    if isdir([data_path,'\Cell',num2str(cell)])
        cd([data_path,'\Cell',num2str(cell)])
           
        load ('imageCube.mat');
        cell_cube = (double(imageCube)-203)./double(cubeParameters.exposure.base);
        norm_cube = double(cell_cube)./double(mirror_cube);
